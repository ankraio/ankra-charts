...
| [`charts-claude-wrapper-image.yml`](.github/workflows/charts-claude-wrapper-image.yml) | Push to `main` under `images/claude-code-openai-wrapper/**` + `workflow_dispatch` | Builds `ghcr.io/ankraio/claude-code-openai-wrapper` (amd64 + arm64) from the upstream commit pinned in the Dockerfile and cosign-signs it. The chart keeps pulling its pinned digest until `sync-image-digest.sh` re-pins it in a reviewed chart release. |
